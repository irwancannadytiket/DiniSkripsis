        ...
        item.btnPlus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(pesananList.size() > 0) {
                    Iterator<Pesanan.Order> iterator = pesananList.iterator();
                    while (iterator.hasNext()) {
                        Pesanan.Order order = iterator.next();
                        if(order.getMenu_id().equalsIgnoreCase(daftarMenuList.getId())) { //udh exist, update bro
                            order.setQty(String.valueOf(value));
                            isExist = true;
                            break;
                        } else { //belum bro
                            isExist = false;
